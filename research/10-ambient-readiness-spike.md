...
| Project Image relation | **Proven** | `WIZARD_PROJECT_IMAGE` resolves to `SPWI703`. The clone carried opcode 237 with parameter 2 (`m_dWFlags`) 2 and `m_sourceId=214305989`, the owner's object ID. The owner had general state 48 plus `SPWI703` lock effects (opcode 233 p1=2/p2=127 and opcode 20). Clones and locked owners can both be skipped without name/resref heuristics. |
